...
export default function ContactPage() {
  const [form, setForm] = useState({ name:'', company:'', email:'', phone:'', countryCode:'+234', subject:'', message:'' });
  const [status, setStatus] = useState<'idle'|'loading'|'success'|'error'>('idle');
  const [validationError, setValidationError] = useState('');
  const handleChange = (e: React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>) =>
    setForm(prev => ({ ...prev, [e.target.name]: e.target.value }));
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setStatus('idle'); setValidationError('');
    
    // Client-side validation
    if (!form.name.trim() || !form.company.trim() || !form.email.trim() || !form.phone.trim() || !form.subject || !form.message.trim()) {
    ...
    try {
      ...
      setStatus('success');
      setForm({ name:'', company:'', email:'', phone:'', countryCode:'+234', subject:'', message:'' });
            ...
            <div className="contact-grid">
              ...
              <div>
                <div className="info-card">
                  ...
                  {[
                    { icon:'📍', label:'Address', value:'No. 7 Ezimgbu Crescent, Presidential Estate, Phase IV, New GRA, Port Harcourt. Rivers State. Nigeria.' },
              ...
              <div className="form-panel">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-row-2">
                    <div className="form-group"><label>Full Name <span style={{color:'red'}}>*</span></label><input name="name" value={form.name} onChange={handleChange} placeholder="John Doe" required /></div>
                    <div className="form-group"><label>Company Name <span style={{color:'red'}}>*</span></label><input name="company" value={form.company} onChange={handleChange} placeholder="" required /></div>
                  </div>
                  <div className="form-row-2">
                    <div className="form-group"><label>Email Address <span style={{color:'red'}}>*</span></label><input type="email" name="email" value={form.email} onChange={handleChange} placeholder="john@example.com" required /></div>
                    <div className="form-group">
                      <label>Phone Number <span style={{color:'red'}}>*</span></label>
                      <div style={{display:'flex'}}>
                        <select name="countryCode" value={form.countryCode} onChange={handleChange} style={{width:'120px', marginRight:'8px'}}>
                        ...
                        <input name="phone" value={form.phone} onChange={handleChange} placeholder="8012345678" required style={{flex:1}} />
                      </div>
                    </div>
                  </div>
                  <div className="form-group">
                  ...
                  <div className="form-group"><label>Message Details <span style={{color:'red'}}>*</span></label><textarea name="message" value={form.message} onChange={handleChange} rows={5} placeholder="Briefly describe your requirements..." required /></div>
